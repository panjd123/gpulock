...
def notify_guard_activity(lock_root: Path, gpu_id: int, mode: str) -> None:
    ...
    # Append a gpulock activity row so short-lived jobs are not missed.
    import sqlite3

    from .activity import init_guard_db, record_gpulock_event

    db_path = lock_root / "guard.db"
    try:
        conn = sqlite3.connect(str(db_path), timeout=1.0)
        init_guard_db(conn)
        record_gpulock_event(conn, gpu_id, now)
        conn.commit()
        conn.close()
    except Exception:
        pass
