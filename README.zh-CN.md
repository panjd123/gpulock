...
- **`check` / `read`** 获取*读锁*。同一张 GPU 上可同时有多个读者持锁。读锁只协调访问，**不预留显存**——多个大显存 `read` 任务仍可能同时 CUDA OOM。请事先规划每张卡最多一个大任务（可加若干小任务），大任务用 `perf`/`write`。
- **`perf` / `write`** 获取*写锁*，它与读者及其他写者均互斥。
- **子进程异常退出报告。** 包装命令非零退出时，`gpulock` 会在 stderr 打印争用报告：活跃锁、近 10 分钟内已释放的会话（避免 peer 先释放导致看不到）、各 holder 估算显存、卡级占用和调度提示。用 `GPULOCK_NO_EXIT_REPORT=1` 可关闭。仅供诊断——任务与 GPU 的分配应在启动前由用户规划好。
